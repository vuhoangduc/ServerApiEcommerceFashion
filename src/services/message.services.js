...
const {
  findById,
  findByIdForShop,
} = require("../services/userSchema.services");
...
class MessageService {
  ...
  static getConversationForShop = async ({ shopId }) => {
    ...
    if (!foundChat || foundChat.length < 0)
      return { message: "Bạn chưa có đoạn chat nào!!!" };
    const userOfChat = [];
    for (let i = 0; i < foundChat.length; i++) {
      const foundUser = await findById(
        converToObjectIdMongodb(foundChat[i].userId)
      );
      chats.push({
        user: {
  ...
  static getConversationForUser = async ({ userId }) => {
    ...
    const userOfChat = [];
    for (let i = 0; i < foundChat.length; i++) {
      const foundUser = await findByIdForShop(
        converToObjectIdMongodb(foundChat[i].shopId)
      );
      if (!foundUser) {
        return {
  ...
  static getMessages = async ({ userId, conversationId }) => {
    const chats = [];
    const foundChat = await conversationSchema
