...
class AdminService {
                ...
                $group: {
                    ...
                    order_products: {
                        $push:
                        {
                            ...
                            priceApplyDiscount: { $first: "$order_products.priceApplyDiscount" },
                        }
                    },
                    item_products: {
                        $push: {
    ...
    static getStatistical = async () => {
        ...
        const topProductSold = await productSchema.find().sort({ product_sold: -1 }).limit(5)
    ...
    static async getShopDetail({ shopId }) {
        ...
        const getProductInShop = await productSchema.findOne({ product_shop: shopId })
        return {
            message: 'Chi tiết cửa hàng',
            shop, storeDetail, getProductInShop
        }
    }
    static unpublishedProductByAdmin = async ({ userId, shopId, content, productId }) => {
    ...
    static getStatisticalShop = async ({ shopId }) => {
        ...
        revenue
        const topProductSold = await productSchema.find({ product_shop: shopId }).limit(5).sort({ product_sold: -1 })
        return {
            message: 'getStatisticalShop',
            revenue, topProductSold,
        }

    }
    static disable = async ({ userId }) => {
        const userDisabled = await userSchema.findOneAndUpdate({ _id: userId }, {
            $set: {
                disable: true,
            }
        })
