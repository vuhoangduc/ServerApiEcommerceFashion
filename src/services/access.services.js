...
class AccessService {
  static verifyOtp = async ({ email, otp, password, role }) => {
  ...
  static signUp = async ({ email, password, role }) => {
    ...
    const transporter = nodemailer.createTransport({
      service: "gmail",
      auth: {
        user: "trustybuyfpt@gmail.com", // Điền email của bạn
        pass: "zonx celh bsjq svaf", // Điền mật khẩu email của bạn hoặc sử dụng ứng dụng mật khẩu
      },
    });
  ...
  static logIn = async ({ email, password, role }) => {
    ...
    if (!hodelUser)
      throw new BadRequestError(
        "Tài khoản chưa tồn tại!",
        StatusCode.FORBIDDEN,
        "INVALID_EMAIL"
      );
    if (hodelUser.disable == true) {
      throw new BadRequestError(
        "Tài khoản của bạn đã bị block hãy liên hệ đến admin để bt thêm chi tiết",
        StatusCode.FORBIDDEN,
        "INVALID_EMAIL"
      );
    }
    if (hodelUser.role != role) {
      throw new BadRequestError(
        "Tài khoản của bạn ko có quyền đăng nhập, hãy tạo tài khoản mới",
        StatusCode.FORBIDDEN,
        "INVALID_EMAIL"
      );
    }
    ...
    const tokens = await createTokenPair(
      { userId: hodelUser._id, email, password: hodelUser.password },
      publicKey,
      privateKey
    );
